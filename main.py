	...
	def __init__(self, SM, mot1, mot2, GP = None, US = None, SM_speed = 1560, starting_point = [4,2], critical_distance = 10, max_map_size = [9,6], turn_tolerance = 0.001, straight_tolerance = 1, motor_speed = 100, motor_speed_turning = 100, block_size = 28):
		...
		def backward():
			...
			if self.backward is not True:
				...
				if calS[0] < len(self.map) and calS[1] < len(self.map[0]) and calS[0] >= 0 and y calS[1]= 0:
					self.map[calS[0]][calS[1]] = self.map_legend["total_block"]
				
				if calL[0] < len(self.map) and calL[1] < len(self.map[0]) and calL[0] >= 0 and y calL[1]= 0:
					self.map[calL[0]][calL[1]] = self.map_legend["total_block"]
				
				if calR[0] < len(self.map) and calR[1] < len(self.map[0]) and calR[0] >= 0 and y calR[1]= 0:
